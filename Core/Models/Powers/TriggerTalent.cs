...
public abstract class TriggerTalent : FigmentTalentPower
{
    protected int TriggerCount { get; private set; }

    /// <summary>
    ///     General-purpose number that this talent would hypothetically pass to a <see cref="FigmentIntent" />.
    ///     Must be positive.
    /// </summary>
    /// <remarks>Callers may use this value for intent labels, tip descriptions, etc.</remarks>
    protected internal virtual decimal? EstimatedAmount => null;
    ...
    private async Task TriggerInternal(PlayerChoiceContext? choiceContext, MoveParams? moveParams)
    {
        ...
        if (player is not null)
        {
            ...
            await Figment.UseAndAdvanceMove(choiceContext, moveParams, TriggerKind.Talent);
        }
    }
}